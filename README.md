...
В репозитории собран полный пайплайн курсовой работы о privacy-рисках после fine-tuning генеративных мультимодальных моделей на задачах document question answering.
...
README сделан подробным, чтобы по нему можно было:
- понять экспериментальный дизайн;
- восстановить структуру артефактов;
- разобраться в разметке полей и coarse field types;
- увидеть, как строятся train/seen/unseen выборки;
- понять, как считаются метрики;
- собрать по проекту методологию, раздел экспериментов и обсуждение результатов.

## 1. Research Questions

В работе четыре основных исследовательских вопроса.
...
Это датасет document QA по одиночным страницам документов. Для каждого примера есть:
- изображение страницы;
- OCR;
- вопрос;
- один или несколько допустимых ответов.

Почему мы взяли именно его:
- он ближе к реальным задачам document AI, чем чисто текстовые privacy-бенчмарки;
- в нем есть поля разной природы:
  ...
  - организации;
  - адреса/контакты;
- он подходит и для pixel-level, и для OCR-level redaction experiments.

### 3.2 Local Data Sources

В проекте используются и локально подготовленные артефакты, и данные из `artifacts/`.
...
Финальная схема разметки, которая используется в проекте, реализована в:
...
Ключевые настройки пайплайна:
- `USE_RULES_FIRST = True`
- `RULE_ACCEPT_THRESHOLD = 0.93`
- `MODEL = "GigaChat"`

То есть rule-based метка принимается сразу только если:
- правило сработало;
- confidence не ниже `0.93`.

Во всех остальных случаях используется разметка через LLM.
